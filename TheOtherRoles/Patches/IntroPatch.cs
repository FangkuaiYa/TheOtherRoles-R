    ...
    public static IEnumerator CoBegin(IntroCutscene __instance)
    {
        ...
        if (GameManager.Instance.IsNormal())
        {
            ...
            else
            {
                ...
                {
                    var parameters =
                        new Il2CppReferenceArray<Il2CppSystem.Object>(new[]
                            { (Il2CppSystem.Object)adjustedNumImpostors });
                    __instance.ImpostorText.text =
        ...
        else
        {
            ...
            var list2 = IntroCutscene.SelectTeamToShow(
                (Func<NetworkedPlayerInfo, bool>)(pcd =>
                    PlayerControl.LocalPlayer.Data.Role.IsImpostor != pcd.Role.IsImpostor)
            );
            ...
            __instance.CrewmateRules.SetActive(false);
            __instance.ImpostorRules.SetActive(false);
            var logicOptionsHnS = GameManager.Instance.LogicOptions as LogicOptionsHnS;
            var logicHnSMusic = GameManager.Instance.GetLogicComponent<LogicHnSMusic>() as LogicHnSMusic;
            if (logicHnSMusic != null) logicHnSMusic.StartMusicWithIntro();
            if (PlayerControl.LocalPlayer.Data.Role.IsImpostor)
            {
                var crewmateLeadTime = (float)logicOptionsHnS.GetCrewmateLeadTime();
                ...
                if (AprilFoolsMode.ShouldHorseAround())
                {
                    ...
                    __instance.HorseWrangleVisualPlayer.UpdateFromPlayerData(PlayerControl.LocalPlayer.Data,
                        PlayerControl.LocalPlayer.CurrentOutfitType, PlayerMaterial.MaskType.None, false);
                }
