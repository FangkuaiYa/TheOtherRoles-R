...
internal class MeetingHudPatch
{
    ...
    private static void swapperConfirm(MeetingHud __instance)
    {
        ...
        if (firstPlayer != null && secondPlayer != null)
        {
            ...
            writer.Write(firstPlayer.PlayerId);
            writer.Write(secondPlayer.PlayerId);
            AmongUsClient.Instance.FinishRpcImmediately(writer);

            RPCProcedure.swapperSwap(firstPlayer.PlayerId, secondPlayer.PlayerId);
                                    ...
                                    if (room != byte.MinValue)
                                        roomName =
                                            DestroyableSingleton<TranslationController>.Instance.GetString(
                                                (SystemTypes)room);
                                    output += "- " + CustomRoleManager.GetRolesString(p, false, false, true) +
