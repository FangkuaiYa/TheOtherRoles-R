...
namespace TheOtherRoles.Roles
{
    public class CustomRoleManager
    {
        ...
        {
            foreach (var role in _roles)
                if (role.CanUseVent(player, vent)) return true;
            return false;
        }
        ...
        {
            foreach (var role in _roles)
                if (role.CanKill(killer, target)) return true;
            return false;
        }
        ...
        public static List<RoleInfo> getRoleInfoForPlayer(PlayerControl p, bool showModifier = true) {
            ...
            if (p == Jackal.jackal || (Jackal.formerJackals != null && Jackal.formerJackals.Any(x => x.PlayerId == p.PlayerId))) infos.Add(Jackal.Info);
        ...
        public static async Task loadReadme() {
            if (ReadmePage == "") {
                ...
                HttpResponseMessage response = await client.GetAsync(Helpers.isChinese()? "https://v6.gh-proxy.org/" : "" + "https://raw.githubusercontent.com/FangkuaiYa/TheOtherRoles-R/main/README.md");
