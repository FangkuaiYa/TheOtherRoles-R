            ...
            if (AmongUsClient.Instance.GameState != InnerNetClient.GameStates.Started)
                if (text.ToLower().StartsWith("/size "))
                    ...
                    if (AmongUsClient.Instance.AmHost && AmongUsClient.Instance.CanBan())
                    {
                        ...
                        {
                            __instance.AddChat(PlayerControl.LocalPlayer, ModTranslation.GetString("DynamicLobbies-Text", 1));
                        }
                        else
                        {
                            ...
                            if (LobbyLimit != GameOptionsManager.Instance.currentNormalGameOptions.MaxPlayers)
                            {
                                ...
                                __instance.AddChat(PlayerControl.LocalPlayer,
                                    string.Format(ModTranslation.GetString("DynamicLobbies-Text", 2), LobbyLimit));
                            }
                            else
                            {
                                __instance.AddChat(PlayerControl.LocalPlayer, string.Format(ModTranslation.GetString("DynamicLobbies-Text", 3), LobbyLimit));
                            }
                        }
                    }
